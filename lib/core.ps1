...
function Invoke-GitLog {
    ...
    process {
        ...
        $format = if ($width -gt 60) {
            ...
            "--format=tformat: * %C(yellow)%h%Creset  $Name%>|($width,trunc)%C(cyan)%cr%Creset%n%w($width - 17,3,5)%s%n"
        }
...
# dirs
function basedir($global) { if ($global) { return $globaldir } $scoopdir -replace '\\$', '' }
